...
@dataclass
class RelationFieldPixelSamplerConfig(PixelSamplerConfig):
    _target: Type = field(default_factory=lambda: RelationFieldPixelSampler)
    num_rays_per_image: int = 256  # different from num_rays_per_batch


class RelationFieldPixelSampler(PixelSampler):
    def __init__(self, config: RelationFieldPixelSamplerConfig, **kwargs):
    ...
    def sample_method(
        ...
        if isinstance(mask, Tensor):
            raise NotImplementedError(
                "RelationFieldPixelSampler does not support masks yet."
            )
