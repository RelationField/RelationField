...
def process_replica(data: Path, output_dir: Path):
    ...
    replica_to_json(copied_image_paths, traj_path, output_dir, data, indices=idx)
...
def replica_to_json(
    ...
    poses_data = np.array(
        [np.array([float(v) for v in p.split()]).reshape((4, 4)) for p in poses_data]
    )
    ...
    out = {
        ...
        "w": w,
        "h": h,
        "ply_file_path": f"{data.name}_mesh.ply",
...
def get_args():
    ...
    parser.add_argument(
        "--data",
        type=str,
        help="Path to the Replica dataset.",
        required=True,
    )
    parser.add_argument(
        "--output_dir",
...
if __name__ == "__main__":
    ...
    for scene in replica.scenes:
        data = Path(f"{args.data}/{scene}")
        output_dir = Path(f"{args.output_dir}/replica_{scene}")
        process_replica(data, output_dir)
